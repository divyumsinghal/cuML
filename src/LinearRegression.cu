...
void LinearRegression<data_type>::fit(std::vector<data_type> h_X,
  ...
  cudaMalloc(&d_X, num_samples * num_features * sizeof(data_type));
  cudaMalloc(&d_y, num_samples * sizeof(data_type));
  cudaMalloc(&d_XTX, num_features * num_features * sizeof(data_type));
  cudaMalloc(&d_XTy, num_features * sizeof(data_type));
  ...
  // Copy data to device
  cudaMemcpy(d_X, h_X.data(), num_samples * num_features * sizeof(data_type),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_y, h_y.data(), num_samples * sizeof(data_type),
             cudaMemcpyHostToDevice);

  const data_type alpha = 1.0f, beta = 0.0f;

  // 1. Compute X^T * X using cublasSgemm
  cublasSgemm(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, num_features,
              num_features, num_samples, &alpha, d_X, num_samples, d_X,
              num_samples, &beta, d_XTX, num_features);

  // 2. Compute X^T * y using cublasSgemv
  cublasSgemv(cublas_handle, CUBLAS_OP_T, num_samples, num_features, &alpha,
              d_X, num_samples, d_y, 1, &beta, d_XTy, 1);

  // 3. Solve (X^T * X) * coefficients = X^T * y using Cholesky decomposition
  cusolverDnSpotrf(cusolver_handle, CUBLAS_FILL_MODE_LOWER, num_features, d_XTX,
                   num_features, d_work, lwork, d_info);

  cusolverDnSpotrs(cusolver_handle, CUBLAS_FILL_MODE_LOWER, num_features, 1,
                   d_XTX, num_features, d_XTy, num_features, d_info);

  // Copy results back
  cudaMemcpy(h_coefficients.data(), d_XTy, num_features * sizeof(data_type),
